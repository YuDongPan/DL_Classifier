...
for fold_num in range(opt.Kf):
    ...
    for testSubject in range(1, opt.Ns + 1):
        ...
        # Define Network
        net = EEGNet.EEGNet(opt.Nc, int(opt.Fs * opt.ws), opt.Nf)
        net = net.to(devices)
...
# 3、Plot Result
Ploter.plot_save_Result(final_acc_list, model_name='EEGNet', dataset='DatasetA', UD=0, ratio=3, win_size=str(opt.ws),
                        text=True)
